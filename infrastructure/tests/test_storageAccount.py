...
class storageAccount(unittest.TestCase):
    ...
    def test_is_tls_set_correctly(self):
        self.assertEqual(
            'TLS1_2',
            self.resource.properties['minimumTlsVersion'],
            'Storage Account is not set to TLS1_2!'
        )

    def test_is_https_accepted_only(self):
        self.assertEqual(
            True,
            self.resource.properties['supportsHttpsTrafficOnly'],
            'Storage account accepts HTTP!'
        )

    def test_is_accessTier_hot(self):
        self.assertEqual(
            "Hot",
            self.resource.properties['accessTier'],
            'Storage account accesTier is not Hot!'
        )

    def test_is_sku_tier_valid(self):
        self.assertEqual(
            "Standard_LRS",
            self.resource.sku.name,
            'Storage account sku tier is not valid!'
        )
