...
class webApp(unittest.TestCase):
    ...
    def test_is_kind_correct(self):
        self.assertEqual("app,linux", self.resource.kind, "Kind is Invalid!")

    def test_is_app_reserved(self):
        self.assertEqual(True, self.resource.properties["reserved"], "Web App not reserved!")

    def test_is_app_free(self):
        self.assertEqual("Free", self.resource.properties["sku"], "Web App is not free!!")
